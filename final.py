...
if 'global_best_model' not in st.session_state:
    st.session_state.global_best_model = None
...
if uploaded_file is not None:
    ...
    if st.button("🚀 Train and Evaluate All Models"):
        ...
        for i, (name, model) in enumerate(models.items()):
            ...
            if accuracy > best_accuracy:
                ...
                global_best_model = model  # Store the actual model object
            elif accuracy == best_accuracy and mcc > best_mcc:
                best_mcc = mcc
                best_model_name = name
                st.session_state.global_best_model = model   # Store the actual model object
    ...
    if st.button("🔮 Predict"):
		if st.session_state.global_best_model is not None:
			# Prepare input data and make predictions
			input_selected = global_selector.transform(input_encoded)
			prediction = st.session_state.global_best_model.predict(input_selected)
			prediction_prob = st.session_state.global_best_model.predict_proba(input_selected)

			if prediction[0] == 1:
				st.success(f"✅ Prediction: Likely to subscribe! Confidence: {prediction_prob[0][1]:.2%}")
			else:
				st.error(f"❌ Prediction: Unlikely to subscribe. Confidence: {prediction_prob[0][0]:.2%}")
		else:
 ...
 # Create input fields for numerical features
    ...
    if st.button("🔮 Predict"):
        if global_best_model is None:
            st.error("Please train the models first by clicking 'Train and Evaluate All Models' button above.")
        else:
...
# Initialize session state for best model name
if 'best_model_name' not in st.session_state:
    st.session_state['best_model_name'] = None
...
if uploaded_file is not None:
    ...
    if st.button("🔮 Predict"):
        if st.session_state.global_best_model is None:
            st.error("Please train the models first by clicking 'Train and Evaluate All Models' button above.")
        else:
            # Create a dictionary with the input values
            input_data = {
